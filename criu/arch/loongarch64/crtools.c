...
int save_task_regs(void *x, user_regs_struct_t *regs, user_fpregs_struct_t *fpregs)
{
	...
	CoreEntry *core = x;
	for (i = 0; i < GPR_NUM; i++)
		assign_reg(core->ti_loongarch64->gpregs, regs, regs[i]);
	assign_reg(core->ti_loongarch64->gpregs, regs, pc);
	
	for (i = 0; i< FPR_NUM; i++)
		assign_reg(core->ti_loongarch64->fpregs, fpregs, regs[i]);
	assign_reg(core->ti_loongarch64->fpregs, fpregs, fcc);
	assign_reg(core->ti_loongarch64->fpregs, fpregs, fcsr);
	return 0;
}
...
int restore_fpu(struct rt_sigframe *sigframe, CoreEntry *core)
{
	...
	memcpy(fpu->regs, core->ti_loongarch64->fpregs->regs, sizeof(fpu->regs));
	fpu->fcc = core->ti_loongarch64->fpregs->fcc;
	fpu->fcsr = core->ti_loongarch64->fpregs->fcsr;
	return 0;
}
